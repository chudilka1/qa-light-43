...
public class SeleniumApiDemo extends BaseTest {
    ...
    @Test
    public void waiters() {
        driver.manage().timeouts().implicitlyWait(5, TimeUnit.SECONDS); //implicit wait (global setting), throws NoSuchElementException
        openIndexPage();
        By usernameSelector = By.name("_usernam");
        //driver.findElement(usernameSelector).getTagName();

        Wait<WebDriver> explicitWait = new WebDriverWait(driver, 20); //excplicit wait that fails by condition
        //explicitWait.until(ExpectedConditions.visibilityOfElementLocated(usernameSelector));

        Duration timeout = Duration.ofSeconds(10);
        Duration pollingTimeout = Duration.ofMillis(200);
        ...
        Wait<WebDriver> wait = new FluentWait<>(driver)
                ...
                .withMessage("Username input was not found");

        wait.until(MyExpectedConditions.myElementDisplayed(usernameSelector));
